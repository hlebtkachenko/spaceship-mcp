...
export function registerSellerHubTools(server: McpServer, ss: SpaceshipClient) {
  server.tool(
    ...
    async ({ take, skip }) => {
      ...
      if (!data?.items?.length) {
        return { content: [{ type: "text", text: "No SellerHub listings." }] };
      }
  ...
  server.tool(
    ...
    async ({ name, displayName, description, binPriceEnabled, binAmount, binCurrency, minPriceEnabled, minAmount, minCurrency }) => {
      ...
      return { content: [{ type: "text", text: `Listed ${name} in SellerHub.` }] };
    },
  );
  ...
  server.tool(
    ...
    async ({ domain, displayName, description, binPriceEnabled, binAmount, binCurrency, minPriceEnabled, minAmount, minCurrency }) => {
      ...
      return { content: [{ type: "text", text: `Updated SellerHub listing for ${domain}.` }] };
    },
  );
  ...
  server.tool(
    ...
    async ({ domain }) => {
      ...
      return { content: [{ type: "text", text: `Removed ${domain} from SellerHub.` }] };
    },
  );
  ...
  server.tool(
    ...
    async () => {
      ...
      if (!data?.options?.length) {
        return { content: [{ type: "text", text: "No verification options." }] };
      }
